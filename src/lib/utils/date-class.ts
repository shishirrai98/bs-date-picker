...
export default class DateClass {
  public dateType: string = "AD";
  public date: DateType = { year: 0, month: 0, day: 0 };
  ...
  setDate(date: Date | string, type: string = "AD"): DateClass {
  ...
  toBS(): string {
    ...
    if (adMonth < 4) {
      ...
      const bsNewYearAdDate = new DateClass(
        this.format({ year: bsYear, month: 1, day: 1 }),
        "BS"
      ).toAD();
    ...
    const bsFirstDayOfMonthAdDate = new Date(
      new DateClass(
        this.format({ year: bsYear, month: bsMonth, day: 1 }),
        "BS"
      ).toAD()
    );
  ...
  totalDaysSince(): number {
    ...
    for (let i = 1; i <= 12; i++) {
      totalDays += this.totalMonthDaysSince(
        i,
        i < bsMonth ? diffInYears + 1 : diffInYears
      );
    }
   ...
   * @param diffInYear
   */
  totalMonthDaysSince(bsMonth: number, diffInYear: number): number {
    if (diffInYear === 0) {
      return 0;
    }
    ...
    const calculated = monthData.slice(0).reduce(
      (
        ...
        if (diffInYear > total.year + monthDataItem) {
          ...
          total.month +=
            Config.bsMonthTotalDaysCount[bsMonth - 1][
              monthTotalDaysCountIndex
            ] * monthDataItem;
        } else {
          total.month +=
            Config.bsMonthTotalDaysCount[bsMonth - 1][
   ...
   * @param bsMonth
   */
  daysInBsMonth(bsYear: number, bsMonth: number): number {
    ...
    for (let i = 0; i < monthData.length; i++) {
      ...
      if (totalYears <= yearCount) {
        if (
          ...
          return Config.bsMonthTotalDaysCount[bsMonth - 1][
            monthTotalDaysCountIndex + 1
          ];
        } else if (bsYear === 2081 && bsMonth === 3) {
          return Config.bsMonthTotalDaysCount[bsMonth - 1][
            monthTotalDaysCountIndex - 1
  ...
  splitDate(date: string): DateType {
  ...
  // noinspection JSMethodCanBeStatic
  splitAdDate(date: Date | string): DateType {
    if (typeof date === "string") {
      date = new Date(date);
    }
  ...
  splitBsDate(date: string): DateType {
  ...
  format(date: DateType): string {
    const separator: string = Config.outputSeparator;
    const { year, month, day } = date;

    return `${year}${separator}${this.zeroPad(month)}${separator}${this.zeroPad(
      day
    )}`;
  }

  // noinspection JSMethodCanBeStatic
  zeroPad(x: number): string {
    return x > 9 ? `${x}` : `0${x}`;
  }
}
