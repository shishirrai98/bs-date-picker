...
export default class DateClass {
    ...
    public date: DateObject = {year: 0, month: 0, day: 0}
    ...
    public setDate(date: Date | string, type: string = "AD"): DateClass {
    ...
    public toBS(): string {
        ...
        if (adMonth < 4) {
            ...
            const bsNewYearAdDate = new DateClass(this.format({year: bsYear, month: 1, day: 1}), "BS").toAD()
        ...
        const bsFirstDayOfMonthAdDate = new Date(
            new DateClass(this.format({year: bsYear, month: bsMonth, day: 1}), "BS").toAD(),
        )
     ...
     * @param diffInYear
     */
    public totalMonthDaysSince(bsMonth: number, diffInYear: number): number {
        if (diffInYear === 0) {
            return 0
        }
     ...
     * @param bsMonth
     */
    public daysInBsMonth(bsYear: number, bsMonth: number): number {
    ...
    public splitDate(date: string): DateObject {
    ...
    // noinspection JSMethodCanBeStatic
    public splitAdDate(date: Date | string): DateObject {
        if (typeof date === "string") {
            date = new Date(date)
        }
    ...
    public splitBsDate(date: string): DateObject {
    ...
    public format(date: DateObject): string {
    ...
    // noinspection JSMethodCanBeStatic
    public zeroPad(x: number): string {
        return x > 9 ? `${x}` : `0${x}`
    }
}
